# -*- coding: utf-8 -*-
"""
Created on Mon Jul 15 15:09:31 2024
...
if config['save']:
    ...
    with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_LAD_weather.pickle', 'wb') as handle:
        pickle.dump(lad, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_Ridge_weather.pickle', 'wb') as handle:
        pickle.dump(ridge, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_Lasso_weather.pickle', 'wb') as handle:
        pickle.dump(lasso, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_MLP_weather.pickle', 'wb') as handle:
...
if config['train']:
    ...
    for number_splits in Max_number_splits:
        ...
        if config['save']:
            with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_FA_lin_greedy_LS_model_{number_splits}_weather.pickle', 'wb') as handle:
...
if config['train']:
    ...
    if config['save']:
        with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_FA_greedy_LS_model_weather.pickle', 'wb') as handle:
...
if config['train']:
    ...
    if config['save']:
        with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_FA_greedy_NN_model_weather.pickle', 'wb') as handle:
...
if config['train']:
    ...
    if config['save']:
        with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_FA_fixed_LS_model_weather.pickle', 'wb') as handle:
...
if config['train']:
    ...
    if config['save']:
        with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_FA_fixed_NN_model_weather.pickle', 'wb') as handle:
...
# ## Old version
# FA_lin_greedy_NN_model = FiniteLinear_MLP(target_col = target_col, fix_col = fix_col, Max_models = 3, D = 1_000, red_threshold = 1e-5, 
#                                             input_size = n_features, hidden_sizes = [], output_size = n_outputs, projection = True, 
#                                             train_adversarially = True, budget_constraint = 'inequality', attack_type = 'greedy')
...
if config['train']:            
    v2FA_lin_greedy_LR_model = Learn_FiniteAdapt_Robust_Reg(target_col = target_col, fix_col = fix_col, Max_models = 3, D = 1_000, red_threshold = 1e-5, 
                                                input_size = n_features, hidden_sizes = [], output_size = n_outputs, projection = True, 
                                                budget_constraint = 'inequality', attack_type = 'greedy', apply_LDR = True)
    
    v2FA_lin_greedy_LR_model.fit(trainPred.values, trainY, val_split = val_perc, tree_grow_algo = 'leaf-wise', max_gap = 1e-3, epochs = num_epochs, patience = patience, verbose = 0, optimizer = 'Adam', 
                         lr = learning_rate, batch_size = batch_size, weight_decay = 1e-5, freeze_weights = False)
        
    if config['save']:
        with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_v2FA_lin_greedy_LR_model.pickle', 'wb') as handle:
            pickle.dump(v2FA_lin_greedy_LR_model, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
if config['train']:
    ...
    if config['save']:
        with open(f'{cd}\\trained-models\\NYISO\\{freq}_{min_lag}_steps\\{target_park}_v2FA_lin_greedy_LR_model_weather.pickle', 'wb') as handle:
