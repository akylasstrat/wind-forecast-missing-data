...
class newversion_FiniteLinear_MLP(object):
  '''This function initializes the GPT.
  ...
  def fit(self, X, Y, tree_grow_algo = 'leaf-wise', max_gap = 0.001, val_split = 0.05, **kwargs):
    ...
    ######## Equality robust/ single feature
    gamma_temp = 1
    eq_robust_model = adjustable_FDR(input_size = num_features, hidden_sizes = self.gd_FDRR_params['hidden_sizes'], output_size = self.gd_FDRR_params['output_size'], 
                              target_col = self.target_features[0], fix_col = self.fixed_features[0], projection = self.gd_FDRR_params['projection'], 
                              train_adversarially = True, 
                              Gamma = gamma_temp, budget_constraint = 'inequality')
    optimizer = torch.optim.Adam(eq_robust_model.parameters(), lr = self.MLP_train_dict['lr'])
    
    # robust_mlp_model.load_state_dict(mlp_model.state_dict(), strict=False)
    eq_robust_model.train_model(train_data_loader, valid_data_loader, optimizer, 
                          epochs = self.MLP_train_dict['epochs'], patience = self.MLP_train_dict['patience'], verbose = self.MLP_train_dict['verbose'], 
                          warm_start = False, attack_type = self.gd_FDRR_params['attack_type'])
    
    ######## Inequality robust/ full budget
    gamma_temp = len(self.target_features[0])
    ineq_robust_model = adjustable_FDR(input_size = num_features, hidden_sizes = self.gd_FDRR_params['hidden_sizes'], output_size = self.gd_FDRR_params['output_size'], 
                              target_col = self.target_features[0], fix_col = self.fixed_features[0], projection = self.gd_FDRR_params['projection'], 
                              train_adversarially = True, Gamma = gamma_temp, budget_constraint = 'inequality')

    optimizer = torch.optim.Adam(ineq_robust_model.parameters(), lr = self.MLP_train_dict['lr'])    
    ineq_robust_model.train_model(train_data_loader, valid_data_loader, optimizer, 
                          epochs = self.MLP_train_dict['epochs'], patience = self.MLP_train_dict['patience'], verbose = self.MLP_train_dict['verbose'], 
                          warm_start = False, attack_type = self.gd_FDRR_params['attack_type'])

    # Nominal and WC loss
    nominal_loss = eval_predictions(mlp_model.predict(train_temp_miss_X), trainY, self.error_metric)
    WC_eq_loss = eval_predictions(eq_robust_model.predict(train_temp_miss_X, self.missing_pattern[0]), trainY, self.error_metric)
    WC_ineq_loss = eval_predictions(ineq_robust_model.predict(train_temp_miss_X, self.missing_pattern[0]), trainY, self.error_metric)
        
    print(f'Nominal loss: {nominal_loss}')
    print(f'Robust Equality loss: {WC_eq_loss}')
    print(f'Upper Bound loss: {WC_ineq_loss}')
    
    # Nominal, WC loss, and loss gap
    self.Loss = [nominal_loss]
    self.WC_eq_Loss = [WC_eq_loss]
    self.WC_ineq_Loss = [WC_ineq_loss]
    
    self.Loss_gap = [self.WC_ineq_Loss[0] - self.Loss[0]]
    self.Loss_gap_perc = [(self.WC_ineq_Loss[0] - self.Loss[0])/self.Loss[0]]
    
    # store nominal and WC model parameters
    # !!!!! Potentially store the weights of a torch model
    self.node_model_ = [mlp_model]
    self.eq_wc_node_model_ = [eq_robust_model]
    self.ineq_wc_node_model_ = [ineq_robust_model]
    ...
    while list_to_check:     
        ...
        for j, cand_feat in enumerate(cand_features):
            ...
            if (current_node_loss > Best_loss)*(nominal_loss_worse_ind):    
                ...
                retrain_loss = eval_predictions(new_mlp_model.predict(train_temp_miss_X), trainY, self.error_metric)
                wc_node_loss = eval_predictions(self.eq_wc_node_model_[node].predict(tensor_train_missX, temp_missing_pattern), trainY, self.error_metric)
        ...
        if apply_split == True:
            ...
            ### Apply binary split: Left partition, selected feature is **not missing**. Right partition, selected feature is **missing**.
            self.total_models += 1
            ...
            # Robust equality model
            eq_left_robust_model = adjustable_FDR(input_size = num_features, hidden_sizes = self.gd_FDRR_params['hidden_sizes'], output_size = self.gd_FDRR_params['output_size'], 
                                      target_col = left_target_cols, fix_col = left_fix_cols, projection = self.gd_FDRR_params['projection'], 
                                      train_adversarially = True, Gamma = 1, budget_constraint = 'inequality')

            optimizer = torch.optim.Adam(eq_left_robust_model.parameters(), lr = self.MLP_train_dict['lr'])
            
            eq_left_robust_model.train_model(left_train_data_loader, left_valid_data_loader, optimizer, 
                                  epochs = self.MLP_train_dict['epochs'], patience = self.MLP_train_dict['patience'], verbose = self.MLP_train_dict['verbose'], 
                                  warm_start = False, attack_type = self.gd_FDRR_params['attack_type'])
            # Estimate WC loss and nominal loss
            left_wc_eq_loss = eval_predictions(eq_left_robust_model.predict(tensor_train_missX, left_missing_pattern), trainY, self.error_metric)

            # Robust inequality model
            gamma_temp = len(left_target_cols)
            ineq_left_robust_model = adjustable_FDR(input_size = num_features, hidden_sizes = self.gd_FDRR_params['hidden_sizes'], output_size = self.gd_FDRR_params['output_size'], 
                                      target_col = left_target_cols, fix_col = left_fix_cols, projection = self.gd_FDRR_params['projection'], 
                                      train_adversarially = True, Gamma = gamma_temp, budget_constraint = 'inequality')

            optimizer = torch.optim.Adam(ineq_left_robust_model.parameters(), lr = self.MLP_train_dict['lr'])
            
            ineq_left_robust_model.train_model(left_train_data_loader, left_valid_data_loader, optimizer, 
                                  epochs = self.MLP_train_dict['epochs'], patience = self.MLP_train_dict['patience'], verbose = self.MLP_train_dict['verbose'], 
                                  warm_start = False, attack_type = self.gd_FDRR_params['attack_type'])
            # Estimate WC loss and nominal loss
            left_wc_ineq_loss = eval_predictions(ineq_left_robust_model.predict(tensor_train_missX, left_missing_pattern), trainY, self.error_metric)

            # Nominal loss: inherits the nominal loss of the parent node/ WC loss: the estimated
            self.Loss.append(self.Loss[node])
            self.WC_eq_Loss.append(left_wc_eq_loss)
            self.WC_ineq_Loss.append(left_wc_ineq_loss)

            self.Loss_gap.append( self.WC_ineq_Loss[-1] -  self.Loss[-1])
            self.Loss_gap_perc.append( (self.WC_ineq_Loss[-1] -  self.Loss[-1])/self.Loss[-1] )
            
            self.missing_pattern.append(left_missing_pattern)
            self.total_missing_feat.append(left_missing_pattern.sum())

            self.node_model_.append(self.node_model_[node])
            self.eq_wc_node_model_.append(eq_left_robust_model)
            self.ineq_wc_node_model_.append(ineq_left_robust_model)
            ...
            # Right equality robust model
            eq_right_robust_model = adjustable_FDR(input_size = num_features, hidden_sizes = self.gd_FDRR_params['hidden_sizes'], output_size = self.gd_FDRR_params['output_size'], 
                                      target_col = right_target_cols, fix_col = right_fix_cols, projection = self.gd_FDRR_params['projection'], 
                                      train_adversarially = True, Gamma = 1, budget_constraint = 'inequality')

            optimizer = torch.optim.Adam(eq_right_robust_model.parameters(), lr = self.MLP_train_dict['lr'])
            
            # right_robust_mlp_model.load_state_dict(self.wc_node_model_[node].state_dict(), strict=False)

            eq_right_robust_model.train_model(right_train_data_loader, right_valid_data_loader, optimizer, 
                                  epochs = self.MLP_train_dict['epochs'], patience = self.MLP_train_dict['patience'], verbose = self.MLP_train_dict['verbose'], 
                                  warm_start = False, attack_type = self.gd_FDRR_params['attack_type'])

            # Estimate WC loss and nominal loss
            right_eq_wc_loss = eval_predictions(eq_right_robust_model.predict(tensor_train_missX, right_missing_pattern), trainY, self.error_metric)
            
            # Right robust inequality loss (use the trained model from the previous node)
            gamma_temp = len(right_target_cols)
            # ineq_right_robust_model = adjustable_FDR(input_size = num_features, hidden_sizes = self.gd_FDRR_params['hidden_sizes'], output_size = self.gd_FDRR_params['output_size'], 
            #                           target_col = right_target_cols, fix_col = right_fix_cols, projection = self.gd_FDRR_params['projection'], 
            #                           train_adversarially = True, Gamma = 1, budget_constraint = 'inequality')

            # optimizer = torch.optim.Adam(ineq_right_robust_model.parameters(), lr = self.MLP_train_dict['lr'])
            
            # # right_robust_mlp_model.load_state_dict(self.wc_node_model_[node].state_dict(), strict=False)

            # ineq_right_robust_model.train_model(right_train_data_loader, right_valid_data_loader, optimizer, 
            #                       epochs = self.MLP_train_dict['epochs'], patience = self.MLP_train_dict['patience'], verbose = self.MLP_train_dict['verbose'], 
            #                       warm_start = False, attack_type = self.gd_FDRR_params['attack_type'])
            
            # # Estimate WC loss and nominal loss
            # right_ineq_wc_loss = eval_predictions(ineq_right_robust_model.predict(tensor_train_missX, right_missing_pattern), trainY, self.error_metric)

            self.Loss.append(Best_insample_loss)
            self.WC_eq_Loss.append(right_eq_wc_loss)
            self.WC_ineq_Loss.append(self.WC_ineq_Loss[node])
            ...
            self.node_model_.append(best_new_model)
            self.eq_wc_node_model_.append(eq_right_robust_model)
            self.ineq_wc_node_model_.append(self.ineq_wc_node_model_[node])
  ...
  def predict(self, X, missing_mask):
     ...
     for i in range(X.shape[0]): 
         ...
         if (m0==self.missing_pattern[node]).all():
             ...
             # Equality robust model
             Predictions.append( self.eq_wc_node_model_[node].predict(x0, m0).reshape(-1))
         else: 
             # Inequality robust model
             Predictions.append( self.ineq_wc_node_model_[node].predict(x0, m0).reshape(-1))
