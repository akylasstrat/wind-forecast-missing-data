...
if config['train']:
    ...
    fin_NN_model.fit(trainPred.values, trainY, val_split = 0.15, tree_grow_algo = 'leaf-wise', max_gap = 0.05, 
...
plt.plot(temp_df['percentage'].unique().astype(float), 100*temp_df.groupby(['percentage'])['NN'].mean().values, 
         linestyle = '--', color = 'black', label = models_to_labels['NN'])

plt.plot(temp_df['percentage'].unique().astype(float), 100*temp_df.groupby(['percentage'])['FinAd-NN'].mean().values, 
         linestyle = '-', color = 'tab:blue', label = 'FinAd-NN')
...
plt.savefig(f'{cd}//plots//{target_park}_nonlinear_RMSE.pdf')
plt.show()