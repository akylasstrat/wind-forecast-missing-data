...
class Finite_LDR_MLP(object):
  ...
  def fit(self, X, Y, tree_grow_algo = 'leaf-wise', max_gap = 0.001, val_split = 0.05, **kwargs):
    ...
    self.children_left_dict = {}
    self.children_right_dict = {}
    self.dimension_alpha = X.shape[1]
    ...
    if self.gd_FDRR_params['hidden_sizes'] == []:
        ...
        # Train Neural Network model
        mlp_model = input_linear_adjustable_FDR(input_size = num_features, hidden_sizes = self.gd_FDRR_params['hidden_sizes'], output_size = self.gd_FDRR_params['output_size'], 
                              target_col = self.target_features[0], fix_col = self.fixed_features[0], projection = self.gd_FDRR_params['projection'], 
                              train_adversarially = True, Gamma = 0, budget_constraint = 'inequality')
    ...
    robust_mlp_model = input_linear_adjustable_FDR(input_size = num_features, hidden_sizes = self.gd_FDRR_params['hidden_sizes'], output_size = self.gd_FDRR_params['output_size'], 
                              target_col = self.target_features[0], fix_col = self.fixed_features[0], projection = self.gd_FDRR_params['projection'], 
                              train_adversarially = True, Gamma = gamma_temp, budget_constraint = 'inequality')
