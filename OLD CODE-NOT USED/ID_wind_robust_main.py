# -*- coding: utf-8 -*-
"""
Checking adaptive linear regression with missing data
...
if config['train']:
    ...
    for K in [0,1]:
        ...
        fdr = FDR_regressor(K = K)
        fdr.fit(trainPred.values, trainY, target_col, fix_col, verbose=-1, solution = 'v-enumeration')  
...
#%%%%% FDDR-CP: vertex-enumeration with cutting plane algorithm
'''
output_file_name = f'{cd}\\{case_folder}\\trained-models\\{target_park}_fdrr-cplane_{max_lag}.pickle'
config['train'] = True
config['save'] = True
if config['train']:
    FDRR_CP_models = []
    for K in K_parameter:
        ...
        fdr = FDR_regressor(K = K)
        fdr.fit(trainPred.values, trainY, target_col, fix_col, verbose = -1, solution = 'cutting plane')  
        
        fdr_pred = fdr.predict(testPred).reshape(-1,1)

        print('FDR: ', eval_point_pred(fdr_pred, Target.values, digits=2))
        FDRR_CP_models.append(fdr)
    
        if config['save']:
            with open(output_file_name, 'wb') as handle:
                pickle.dump(FDRR_CP_models, handle)
else:
    with open(output_file_name, 'rb') as handle:    
            FDRR_CP_models = pickle.load(handle)
'''
...
plt.xticks(np.arange(trainPred.shape[1]), trainPred.columns, rotation = 90)
plt.show()
...
error_fdr = Target.values - final_fdr_aar_pred
error_lad = Target.values - lad_pred

error_df = pd.DataFrame(data = miss_X.isna().sum(1), columns = ['num_na'])
error_df['LAD'] = np.abs(error_lad)
error_df['FDRR'] = np.abs(error_fdr)

error_df.groupby('num_na').mean().plot()
plt.show()

error_df['num_na'].hist(bins = 20)
plt.show()
#%%
plt.scatter(num_na, np.abs(error_lad))
plt.scatter(num_na, np.abs(error_fdr))
plt.show()
#%%
