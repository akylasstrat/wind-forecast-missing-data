# -*- coding: utf-8 -*-
"""
Pareto optimal solutions check
...
for k in range(3):
    ...
    fdr = FDR_regressor(K = K_parameter)
    ...
    fdr_test = FDR_regressor_test(K = K_parameter)
    fdr_test.fit(trainPred.values, trainY, target_col, fix_col, verbose=-1, solution = 'reformulation', budget = 'equality')  
    
    print('FDR: ', eval_point_pred(fdr_test.predict(testPred).reshape(-1,1), Target.values, digits=2))
    
    plt.plot(fdr.coef_)
    plt.plot(fdr_test.coef_, '--')
    plt.show()

#%%

fdr_test = FDR_regressor_test(K = K_parameter)
fdr_test.fit_PRO(trainPred.values, trainY, target_col, fix_col, verbose=-1, solution = 'affine')  
...
fdr = FDR_regressor(K = K_parameter)
fdr.fit(trainPred.values[:,:4], trainY, target_col[:4], fix_col, verbose=-1, solution = 'v-enumeration')  

fdr_test = FDR_regressor_test(K = K_parameter)
fdr_test.fit_PRO(trainPred.values[:,:4], trainY, target_col [:4], fix_col, verbose=-1, solution = 'v-enumeration')  
...
#%%
fdr_test = FDR_regressor_test(K = K_parameter)
fdr_test.fit_PRO(trainPred.values, trainY, target_col, fix_col, verbose=-1, solution = 'reformulation')  

plt.plot(fdr.coef_)
plt.plot(fdr_test.coef_)
plt.show()
#%% Check for parreto optimal solutions

t_wc_objval = fdr.objval
# find an ''average'' case, by random sampling of missing data 
random_missing_ind = np.random.choice(np.arange(len(target_col)), (trainPred.shape[0], K_parameter))
K = K_parameter

n_train_obs = trainPred.shape[0]
n_feat = trainPred.shape[1]
X = trainPred.values.copy()

alpha_ave = np.zeros(trainPred.shape)
for  i in range(trainPred.shape[0]): alpha_ave[i,random_missing_ind[i]] = 1
...
wc_loss = m.addMVar(n_train_obs, vtype = gp.GRB.CONTINUOUS, lb = 0, name = 'wc_loss')
...
m.addConstr( wc_loss >= .5*(trainY.reshape(-1) - fitted + t_up))
m.addConstr( wc_loss >= (1-.5)*(-trainY.reshape(-1) + fitted + t_down))

m.addConstr( (1/n_train_obs)*wc_loss.sum() <= t_wc_objval)

# Average Objective
m.addConstr( ave_loss >= .5*(trainY.reshape(-1) - (X*(1-alpha_ave))@coef ))
m.addConstr( ave_loss >= (1-.5)*(-trainY.reshape(-1) + (X*(1-alpha_ave))@coef ))
...
output_file_name = f'{cd}\\{case_folder}\\trained-models\\{target_park}_fdrr-r_{max_lag}.pickle'
...
#%%%%%%%%% FDRR-PWL: piecewise linear approximation of the quadratic loss
from FDR_PWL_regressor import *

c = np.arange(1,10)
error = np.arange(-1, 1+.1,.1).round(1)
plt.plot(error*error)
#plt.plot(np.abs(error))
step = .5
x_0 = np.arange(-1, 1 + step, step)
C = []
B = []
for i in range(len(x_0)):
    if i ==0:continue
    c = ((x_0[i]**2-x_0[i-1]**2)/step).round(2)
    b = (x_0[i]**2 - c*x_0[i]).round(2) 
    plt.plot(c*error + b)
    C.append(c)
    B.append(b/c)
plt.ylim([-.1, 1.2])
...
output_file_name = f'{cd}\\{case_folder}\\trained-models\\{target_park}_fdrr-pwl_{max_lag}.pickle'
...
if config['train']:
    ...
    for K in K_parameter:
        ...
        fdr = FDR_PWL_regressor(K = K, c = C, b = B)
        fdr.fit(trainPred, trainY, target_col, fix_col, verbose=-1, solution = 'reformulation')  
        
        fdr_pred = fdr.predict(testPred).reshape(-1,1)

        print('FDR: ', eval_point_pred(fdr_pred, Target.values, digits=2))
        FDRR_PWL_models.append(fdr)
    
    if config['save']:
        with open(output_file_name, 'wb') as handle:
            pickle.dump(FDRR_PWL_models, handle)
else:
    with open(output_file_name, 'rb') as handle:    
            FDRR_PWL_models = pickle.load(handle)
#%%
t = 10
for i in range(1,t):
    plt.plot(FDRR_R_models[i].coef_)
    plt.plot(FDRR_AAR_models[i].coef_)
    plt.show()
...
models = ['PERS', 'LS', 'LS-l2', 'LS-l1', 'LAD', 'LAD-l1','FDRR-R', 'FDRR-AAR']
...
for perc in percentage:
    ...
    for iter_ in range(iterations):
        ...
        fdr_r_mae = eval_predictions(final_fdr_r_pred, Target.values, metric= error_metric)

        #### FDRR-PWL (select the appropriate model for each case)
        fdr_pwl_predictions = []
        for i, k in enumerate(K_parameter):
            fdr_pred = FDRR_PWL_models[i].predict(miss_X_zero).reshape(-1,1)
            fdr_pred = projection(fdr_pred)
            # Robust
            if config['scale']: fdr_pred = target_scaler.inverse_transform(fdr_pred)
            fdr_pwl_predictions.append(fdr_pred.reshape(-1))
        fdr_pwl_predictions = np.array(fdr_pwl_predictions).T
        # Use only the model with the appropriate K
        final_fdr_pwl_pred = fdr_pwl_predictions[:,0]
        if (perc>0)or(config['pattern']=='MNAR'):
            for j, ind in enumerate(mask_ind):
                n_miss_feat = miss_X.isna().values[j].sum()
                final_fdr_pwl_pred[j] = fdr_pwl_predictions[j, n_miss_feat]
        final_fdr_pwl_pred = final_fdr_pwl_pred.reshape(-1,1)
        
        fdr_pwl_mae = eval_predictions(final_fdr_pwl_pred, Target.values, metric= error_metric)
...
models_to_plot = ['LS', 'LS-l2', 'LS-l1', 'LAD', 'LAD-l1', 'FDRR-R', 'FDRR-PWL']

ls_colors = plt.cm.tab20c( list(np.arange(3)))
lad_colors = plt.cm.tab20( list(np.arange(10,11)))
fdr_colors = plt.cm.tab20c([8,9,10, 12, 13, 14])

colors = list(ls_colors) + ['tab:brown'] + ['black'] + list(fdr_colors) 

std_bar = mae_df.groupby(['percentage'])[models_to_plot].std()
fig,ax = plt.subplots(constrained_layout = True)
mae_df.groupby(['percentage'])[models_to_plot].mean().plot(kind='bar', ax=ax, rot = 0, 
                                                 yerr=std_bar, legend=True)

plt.hlines(persistence_mae, 0, 6, color = 'black', label = 'Persistence')
plt.legend()
ax.set_ylabel('MAE')
ax.set_xlabel('Transition Probability')
plt.show()
#%%
models_to_plot = ['PERS', 'LS', 'LS-l1', 'LAD', 'FDRR-AAR']
marker = ['2', 'o', 'd', '^', '8', '1', '+', 's', 'v', '*', '^', 'p']
labels = ['$\mathtt{PERS}$', '$\mathtt{LS}$', '$\mathtt{LS}_{\ell_2}$', '$\mathtt{LS}_{\ell_1}$',
           '$\mathtt{LAD}$', '$\mathtt{LAD}_{\ell_1}$','$\mathtt{FDRR}$', '$\mathtt{FDRR-AAR}$']
...
#ax.set_xscale('log')
if config['save']: 
    plt.savefig(f'{cd}\\{case_folder}\\figures\\{target_park}_ID_mae.pdf')
plt.show()
