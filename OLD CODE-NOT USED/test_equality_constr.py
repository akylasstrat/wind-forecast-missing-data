# -*- coding: utf-8 -*-
"""
test relaxing the equality constraint 
...
for K in K_parameter:
    ...
    fdr.fit(trainPred.values, trainY, target_col, fix_col, verbose=-1, solution = 'reformulation', budget = 'equality')  
    ...
    fdr = FDR_regressor_test(K = K)
    fdr.fit(trainPred.values, trainY, target_col, fix_col, verbose=-1, solution = 'reformulation', budget = 'inequality')  
    fdr_pred = fdr.predict(testPred).reshape(-1,1)

    print('FDR: ', eval_point_pred(fdr_pred, Target.values, digits=2))
    FDRR_R_test_models.append(fdr)

    plt.plot(FDRR_R_models[K].coef_)
    plt.plot(FDRR_R_test_models[K].coef_)
    plt.title(f'K={K}')
    plt.show()

#%%

plt.plot([FDRR_R_models[K].objval for K in K_parameter])
plt.show()
...
for i in range(trainPred.shape[1]):
...
models = ['PERS', 'LS', 'LS-l2', 'LS-l1', 'LAD', 'LAD-l1','FDRR-R', 'FDRR-R-test']
labels = ['$\mathtt{PERS}$', '$\mathtt{LS}$', '$\mathtt{LS}_{\ell_2}$', '$\mathtt{LS}_{\ell_1}$',
           '$\mathtt{LAD}$', '$\mathtt{LAD}_{\ell_1}$','$\mathtt{FDRR-R}$', '$\mathtt{FDRR-R-test}$']
...
for perc in percentage:
    ...
    for iter_ in range(iterations):
        ...
        #### FDRR-R -test
        fdr_r_test_predictions = []
        for i, k in enumerate(K_parameter):
            fdr_pred = FDRR_R_test_models[i].predict(miss_X_zero).reshape(-1,1)
            fdr_pred = projection(fdr_pred)
            # Robust
            if config['scale']: fdr_pred = target_scaler.inverse_transform(fdr_pred)
            fdr_r_test_predictions.append(fdr_pred.reshape(-1))
        fdr_r_test_predictions = np.array(fdr_r_test_predictions).T
        # Use only the model with the appropriate K
        final_fdr_r_pred = fdr_r_test_predictions[:,0]
        if (perc>0)or(config['pattern']=='MNAR'):
            for j, ind in enumerate(mask_ind):
                n_miss_feat = miss_X.isna().values[j].sum()
                final_fdr_r_pred[j] = fdr_r_test_predictions[j, n_miss_feat]
        final_fdr_r_pred = final_fdr_r_pred.reshape(-1,1)
        
        fdr_r_test_mae = eval_predictions(final_fdr_r_pred, Target.values, metric= error_metric)
        ...
        temp_df['FDRR-R-test'] = fdr_r_test_mae
...
models_to_plot = ['LS', 'LS-l2', 'LS-l1', 'LAD', 'LAD-l1', 'FDRR-R', 'FDRR-R-test']
