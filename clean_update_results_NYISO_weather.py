# -*- coding: utf-8 -*-
"""
Updating existing results (does not re-run everything)
...
models_to_add = ['FA_LEARN_LDR_LR_model_1', 
                 'FA_LEARN_LDR_LR_model_2',
                 'FA_LEARN_LDR_LR_model_5']

models_dict = {}
### FA - LEARN - LDR - LR/NN
for m in models_to_add:
    with open(f'{trained_models_path}\\{target_park}_{m}_weather.pickle', 'rb') as handle:
        models_dict[m] = pickle.load(handle)
...
for perc in percentage:
    for iter_ in range(iterations):
        ...
        for m in models_to_add:
            ...
            temp_pred = models_dict[m].predict(miss_X_zero.values, miss_X.isna().values.astype(int))
            temp_pred = projection(temp_pred).reshape(-1)
            
            mae_df.loc[(mae_df['percentage'] == perc)* (mae_df['iteration'] == iter_), m] = mae(temp_pred[max_lag-1:], Target.values[max_lag-1:])
            rmse_df.loc[(rmse_df['percentage'] == perc)* (rmse_df['iteration'] == iter_), m] = rmse(temp_pred[max_lag-1:], Target.values[max_lag-1:])

        run_counter += 1
#%%
    ls_models = ['LR'] + models_to_add
